...
def build_model(W, N, widths, heights):
    ...
    prob += l, "Mininum height of the chip"
    ...
    for i in set_N:
        for j in set_N:
            if i < j:
                ...
                prob += coord_y[i] + heights[i] <= coord_y[j] + delta[i][j][1] * W
                prob += coord_x[j] + widths[j] <= coord_x[i] + delta[i][j][2] * l_up
...
if __name__ == "__main__":
    W, N, widths, heights = extract_input_from_txt(data_path["txt"], "ins-40.txt")
    prob = build_model(W, N, widths, heights)
    solver = CPLEX_PY(timeLimit=300)
    prob.solve(solver)
    print("Status: ", pl.LpStatus[prob.status])
    print("Height: ", pl.value(prob.objective)) 
